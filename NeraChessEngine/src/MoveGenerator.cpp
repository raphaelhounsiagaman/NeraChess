...
namespace NeraChessEngine
{
	...
	const std::array<uint64_t, 64> MoveGenerator::s_RookMagics = { 9547631778034934032ULL, 2540031495509114944, 144126734022869024, 72066527644352768, 684573532780249728, 3602897590569673216, 72078484826206720, 72057733705170946, 3459468203411914752, 6896411809415808, 4902449739273863425, 405886987824798210, 9223513066409334784ULL, 72198348706283904, 38843583326863620, 38843583326863620, 9223654611888374080ULL, 9642992121978880, 1157425241740804256, 145110246652645376, 2534374570854400, 576602039648257024, 36033195100078344, 76572188787146884, 600893842079744, 5206477829663883338, 40549990981959808, 36037608145420416, 72629344379666768, 2305988146896175232, 10377718525713713705ULL, 145294989822300484, 578853564760195616, 189221555258008840, 576534432752607232, 145110246652645376, 28297033425093632, 18436619589653504, 38843583326863620, 2954361909639381060, 2323927783189151745, 72444627768098824, 54397240455856144, 1157425138602377232, 36037593179127936, 576480543579865216, 324260272715890816, 3387131808579585, 6917819574402416896, 6896411809415808, 40549990981959808, 36037608145420416, 141459043123328, 72198348706283904, 571754662593536, 4400198257152, 4575068227109409, 37295511727907078, 1196406106886169, 914828308643842, 2306125910396895233, 54324825124506121, 9223976802882457860ULL, 1019009924040228930 };
	const std::array<uint64_t, 64> MoveGenerator::s_BishopMagics = { 148619892600473088, 290484652082266500, 148619892600473088, 2306973309314531344, 1450728695758062096, 4945238401398865952, 437139439488614424, 792740221539328514, 1497672410529856, 9150447170093186, 44057977971264, 612491791328904194, 6341503701806481409, 220676965928140802, 1441155183740469504, 76567799333651456, 6935543632376629637, 9150447170093186, 2251868535308308, 146403289020272640, 9225659038254498050ULL, 4612249010261073921, 38027726347060226, 4661955075971080, 153131252688831501, 294440452730650768, 862017267433506, 144704528522952712, 4611967562140368904, 72567784243720, 2743255397968316450, 8358964582840026112, 2306705301418616064, 77704703491875841, 72128102369591554, 9042417988731140, 576553128460042496, 1153486112418760738, 148619892600473088, 290484652082266500, 1234417875809222656, 81570824520768, 1252073270656765956, 432345703025674242, 36029905154081824, 166668405082965025, 571754662593536, 307375090601820224, 437139439488614424, 2306425202970460544, 218570002072110338, 290484652082266500, 70437497344024, 22572983652515856, 578855505878059096, 290484652082266500, 792740221539328514, 76567799333651456, 4611686022923879428, 108088590088799232, 35185446160642, 2251954567779072, 1497672410529856, 148619892600473088 };
	...
	const MoveList<218>& MoveGenerator::GenerateMoves(const BoardState& board, bool capturesOnly)
	{
		m_BoardState = board;
		m_CapturesOnly = capturesOnly;
	...
	void MoveGenerator::InitGen()
	{
		...
		// Only restricts emission when not in check -- an in-check position needs
		// its full evasion list regardless of what the caller asked for, since
		// capturesOnly has no way to represent "no captures, but not stalemate".
		m_CaptureFilter = (m_CapturesOnly && !m_InCheck) ? m_OpponentPieces : ~0ULL;
	}
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (m_AllPieces & (1ULL << squareIndex))
				{
					...
					else
					{
						...
						if ((isDiagonal && piece.IsDiagonalSlider()) ||
							(!isDiagonal && piece.IsOrthogonalSlider()))
						{
	...
	{
		Bitboard legalMask = ~(m_OpponentAttackMap | m_FriendlyPieces) & m_CaptureFilter;
		...
		// Castling never captures, so it has no place in a captures-only list.
		if (m_InCheck || m_CapturesOnly)
			return;
	...
	{
		Bitboard moveMask = ~m_FriendlyPieces & m_CheckRayBitmask & m_CaptureFilter;
	...
	void MoveGenerator::CalculatePawnMoves()
	{
		...
		// A capturesOnly generation (only requested when not in check) has no use
		// for a quiet single or double push -- neither can ever be a capture, and
		// push promotions (kept below) already cover the one way a pawn push
		// stays a quiescence candidate.
		if (!m_CapturesOnly || m_InCheck)
		{
